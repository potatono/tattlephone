...
class Tattlephone:
    ...
    def start(self):
        while True:
            ...
            if self.now() >= self.next_call:
                if not self.is_on_hook():
                    print("Cannot call, phone is still in use.")
                    self.call_in(config.call_mins_after_tattle)

                elif self.is_calling_hour() or self.next_call_afterhours:
                    print("Calling!")
    ...
    def is_on_hook(self):
        cmd = "asterisk -rx \"pjsip show endpoint {}\" | grep \"Not in use\"".format(config.phone_extension)
